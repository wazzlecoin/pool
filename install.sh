...
# Maintained by: imskaa
# Version 1.9 (January 2020)
#
# Program:
#   Install Wazzle Pool on Ubuntu 18.04 running Nginx, MariaDB, and php7.x
# 
################################################################################
output() {
    printf "\E[0;33;40m"
...
' | sudo -E tee /etc/nginx/blockuseragents.rules >/dev/null 2>&1
    ...
    cd ~
    git clone https://github.com/wazzle/pool.git yiimp
...
cd ~
wget https://github.com/wazzle/lbrycrd/releases/download/v0.17.3.2/lbrycrd-linux-1732.zip
...
# Create config for Wazzle
echo && echo "Configuring Wazzlecrd.conf"
sleep 3
